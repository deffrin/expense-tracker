                                ...
                                <tbody class="group text-sm text-stone-800 ">
                                    @foreach ($categories as $category)
                                        <tr class="border-b border-stone-200 last:border-0">
                                            ...
                                            <td class="p-3">
                                                {{ \Carbon\Carbon::create()->month((int) $selectedMonth)->format('F') . ', ' . $selectedYear }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            @else
                <div role="alert"
                    ...
                    <div class="w-full text-sm font-sans leading-none m-1.5 flex justify-between items-center">
                        You don't have any expenses for this month {{ $selectedCategory ? 'in this category' : '' }}.
                    </div>
                </div>
            @endif
        </div>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                script.onload = () => {
                    ...
                    new Chart(ctx, {
                        type: 'line',
                        data: {
                            labels: @json($categories->pluck('name')->toArray()),
                            datasets: [{
                                label: 'Expenses',
                                data: @json($categories->pluck('total')->map(fn($total) => (int) ($total ?? 0))->values()),
                                borderColor: '#22C55E',
                        ...
                        options: {
                            ...
                            scales: {
                                ...
                                y: {
                                    min: 0,
                                    max: @json($categories->max('total') + 100),
                                    grid: {
                                        color: '#F3F4F6',
                                    },
                                    ticks: {
